...
function App() {
  ...
  function adpc(index,pk,gr,at,df){
    if(p1Party.length >= 5){
      alert('파티는 최대 5명 까지임');
      return;
    }
    console.log(`보유카드 번호: ${index}`);
    var p = {pokemon: pk, grade: gr, attack: at, defense: df};
  ...
  var getMyPartyApi = useCallback(()=>{
    axios.get('http://localhost:8080/spring/card/getMyParty')
    .then(response=>{
  ...
  return (
    <>
      <h2>플레이어1</h2>
      <button onClick={clearPartyApi}>파티 삭제</button>
      <CardArea>
      ...
      <CardArea>
        {player1.map((cd, index) => (
          <Card key={index} pokemon={cd.pokemon} grade={cd.grade} at={cd.attack} df={cd.defense}
            adp={()=>adpc(index,cd.pokemon,cd.grade,cd.attack,cd.defense)} />
        ))}
      </CardArea>
      <fieldset>
        <legend>상점</legend>
